...
public class ColdcardHsmPolicyTests
{
	...
	{
		// Firmware predating a field rejects the whole policy over the unknown key, so absent must mean
		// absent. A zero would also read as "allow nothing" if it were accepted.
		var json = ColdcardHsmPolicy.Compose([Segwit], new ColdcardHsmPolicy.ColdcardLimits(
			MinSelfTransferPercent: 99.0,
			MaxSatsLeaving: null,
