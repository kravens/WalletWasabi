...
public class CoinJoinManager : BackgroundService
{
	...
	/// behave identically. The limits the user confirms - the fee cap, and for a device policy the fewest
	/// inputs a round must have - travel with the key chain into every round, where the per-round checks in
	/// CoinJoinClient skip what the device would refuse before any input is registered.
	/// </summary>
