...
public class KruxKeyChain : IKeyChain, IDisposable
{
	...
	/// What the device's approved session still allows, as the bridge reported it. Zero or less means the
	/// budget is already spent; a negative <paramref name="roundsRemaining"/> is not expected but is treated
	/// the same way. The device enforces its own budget regardless; this is tracked so that a coinjoin is not
	/// started on a session that has nothing left, which would fail only once it came time to sign.
	/// </param>
	public KruxKeyChain(KruxClient client, KeyManager keyManager, int roundsRemaining = int.MaxValue)
	{
