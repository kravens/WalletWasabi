...
public static class HardwareCoinJoin
{
	...
	public static HardwareCoinJoinVendor GetCoinJoinVendor(this KeyManager keyManager)
	{
		...
		return keyManager.IsTrezorCoinJoinWallet() ? HardwareCoinJoinVendor.Trezor : HardwareCoinJoinVendor.None;
	}
	...
	/// <summary>
	/// True when the vendor keeps coinjoin funds in a separate SLIP-25 account (Trezor), rather than signing
	/// from the wallet's default accounts under a device-side policy (Coldcard). This drives the account
	/// splitting rules, so it is asked about the account model rather than about the brand.
	/// </summary>
	public static bool UsesSlip25CoinJoinAccount(this KeyManager keyManager) =>
		keyManager.IsTrezorCoinJoinWallet();
}
